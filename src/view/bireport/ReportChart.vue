...
export default {
  ...
  render (h) {
    ...
    //切换图表
    let cgbtn = h('button', {
      attrs: {class: 'btn btn-block btn-default'}, on: {
        click: () => {
    ...
    if (isscatter) {
      ...
      if (comp.kpiJson && comp.kpiJson.length > 0 && comp.kpiJson[0]) {
        ...
        xcol = [h('span', {
          ...
          attrs: {class: 'charticon'},
          domProps: {innerHTML: `<i class="fa fa-gear"></i>`},
          on: {click: () => this.chartmenu(o, 'ycol')}
        })]
      } else {
        xcol = [h('span', {class: 'charttip'}, '将度量拖到这里')]
      }
      //横轴　
      let xcolobj = h('div', {class: 'ts_h'}, [h('div', '横轴：'), h('div', {
        attrs: {
          class: 'h_ctx',
          id: 'ycol'
        }
      ...
      if (comp.kpiJson && comp.kpiJson.length > 0 && comp.kpiJson[1]) {
        ...
        ycol = [h('span', {
          ...
          attrs: {class: 'charticon'},
          domProps: {innerHTML: `<i class="fa fa-gear"></i>`},
          on: {click: () => this.chartmenu(o, 'y2col')}
        })]
      } else {
        ycol = [h('span', {class: 'charttip'}, '将度量拖到这里')]
      }
      let ycolobj = h('div', {class: 'ts_h'}, [h('div', '纵轴'), h('div', {
        attrs: {
          class: 'h_ctx',
          id: 'y2col'
        }
      ...
      if (isbubble) {
        ...
        let qpobj = h('div', {class: 'ts_h'}, [h('div', '气泡大小'), h('div', {
          attrs: {
            class: 'h_ctx',
            id: 'y3col'
          }
    ...
    if (comp.chartJson && comp.chartJson.xcol) {
      ...
      xcol = [h('span', {attrs: {class: 'charttxt', title: o.dimdesc}}, o.dimdesc), h('a', {
        attrs: {class: 'charticon'},
        domProps: {innerHTML: `<i class="fa fa-gear"></i>`},
        on: {click: () => this.chartmenu(o, 'xcol')}
      })]
    } else {
      xcol = [h('span', {class: 'charttip'}, '将维度拖到这里')]
    }
    //横轴
    let xcolobj = h('div', {class: 'ts_h'}, [h('div', isscatter ? '观察维度：' : '横轴：'), h('div', {
      attrs: {
        class: 'h_ctx',
        id: 'xcol'
      }
    ...
    if (tp !== 'map') {  //地图没有图例　
      ...
      if (comp.chartJson && comp.chartJson.scol) {
        ...
        scol = [h('span', {
          ...
          attrs: {class: 'charticon'},
          domProps: {innerHTML: `<i class="fa fa-gear"></i>`},
          on: {click: () => this.chartmenu(o, 'scol')}
        })]
      } else {
        scol = [h('span', {class: 'charttip'}, '将维度拖到这里')]
      }
      // 图例 Ser
      let scolobj = h('div', {class: 'ts_h'}, [h('div', '图例'), h('div', {
        attrs: {
          class: 'h_ctx',
          id: 'scol'
        }
    ...
    let exchange = [
      ...
      }, [h('img', {attrs: {src: require('../../assets/image/reload.png')}})]),
    ...
    if (comp.chartJson.params && comp.chartJson.params.length > 0) {
      ...
      $(comp.chartJson.params).each((a, b) => {
        clds.push(h('span', {class: 'chartdrillDim'}, [h('a', {
          class: 'fa fa-minus-square-o',
          attrs: {href: 'javascript:;', title: '上卷'},
          on: {
            click: () => {
              chartUtils.chartGoupDim(comp, b.id, b.pos, () => {
                this.chartView()
              })
            }
          }
    ...
    return h('div', [r, h('div', {
      attrs: {id: 'chartDirllBtn'},
      style: {display: 'block'}
    }, ''), h('chgChartDailog', {ref: 'chgChartForm'}, ''), h('reportChartDailog', {ref: 'reportChartForm'}, '')])
  },
  mounted () {
      ...
      $('#T' + id + ' #xcol, #T' + id + ' #ycol, #T' + id + ' #y2col, #T' + id + ' #y3col, #T' + id + ' #ycols, #T' + id + ' #scol').droppable({
        ...
        drop: function (e, ui) {
          ...
          if (node.li_attr.col_type == 2 && (dropId == 'ycol' || dropId == 'y2col' || dropId == 'y3col')) {
            let o = {
              'kpi_id': node.li_attr.col_id,
              'kpi_name': node.text,
              'col_name': node.li_attr.col_name,
              'aggre': node.li_attr.aggre,
              'fmt': node.li_attr.fmt,
              'alias': node.li_attr.alias,
              'tname': node.li_attr.tname,
              'unit': node.li_attr.unit,
              'rate': node.li_attr.rate,
              'calc': node.li_attr.calc
            }
            if (dropId === 'ycol') {
              json.kpiJson[0] = o
          ...
          if (node.li_attr.col_type == 1 && dropId == 'xcol') {
            ...
            json.chartJson.xcol = {
              'id': node.li_attr.col_id,
              'dimdesc': node.text,
              'type': node.li_attr.dim_type,
              'colname': node.li_attr.col_name,
              'alias': node.li_attr.alias,
              'tname': node.li_attr.tname,
              'iscas': node.li_attr.iscas,
              'tableName': node.li_attr.tableName,
              'tableColKey': node.li_attr.tableColKey,
              'tableColName': node.li_attr.tableColName,
              'dimord': node.li_attr.dimord,
              'dim_name': node.li_attr.dim_name,
              'grouptype': node.li_attr.grouptype,
              'valType': node.li_attr.valType,
              'ordcol': node.li_attr.ordcol,
              dateformat: node.li_attr.dateformat,
              'calc': node.li_attr.calc
            }
            ts.setUpdate()
            ts.chartView()
      ...
      ajax({
        ...
        success: (resp) => {
          ...
          var execf = () => {
            ...
            if (!myChart) {
              myChart = echarts.init(document.getElementById('chart' + this.chartId), 'default', {
                width: 640,
                height: 320
              })
            }
    ...
    chartmenu (o, pos) {
      ...
      $.contextMenu({
        ...
        items: {
          ...
          'top': {
            name: '取Top...', disabled: function (key, opt) {
              if (pos == 'xcol') {
                return false
              } else {
                return true
          ...
          'prop': {
            name: '属性...', disabled: function (key, opt) {
              let tp = pos
      ...
      var opts = function (resp) {
        ...
        for (let i = 0; i < resp.length; i++) {
          ...
          if (resp[i].grouptype == '' || resp[i].grouptype == null) { //无分组的，直接显示维度
            ...
            items['dim_' + id] = {
              name: '<span style="color:#ccc">下钻</span>' + resp[i].dim_desc,
              isHtmlName: true,
            ...
            if (!groupExist(ignoreGroup, resp[i].grouptype)) {
              ...
              var o = items['g' + resp[i].grouptype] = {
                name: '<span style="color:#ccc">下钻</span>' + resp[i].groupname,
                isHtmlName: true,
                items: {}
              }
              ...
              for (let kl = 0; kl < lsdim.length; kl++) {
                ...
                if (!cz) {
                  o.items['dim_' + tmp.dim_id] = {
                    name: '<span style="color:#ccc">下钻</span>' + tmp.dim_desc,
                    isHtmlName: true,
                    callback: function (itemKey, opt, e) {
                      var dimid = itemKey.split('_')[1]
                      chartUtils.drillingChart(dimid, comp, 'row', xvalue, oldDimId, () => {
