          ...
          <template v-for="(item) in menus">
            <el-submenu v-if="item.children&&item.children.length>0" :key="item.menuId" :index=" 'p' + item.menuId">
              ...
              <template v-for="(sub) in item.children">
                ...
                <template v-else>
                  <el-menu-item :index="sub.menuUrl" :key="sub.menuId"><i :class="sub.avatar"></i>{{ sub.menuName }}
                  </el-menu-item>
                </template>
              </template>
            </el-submenu>
            <el-menu-item v-else :key="item.menuId" :index="item.menuUrl"><i
              :class="item.avatar"></i>{{ item.menuName }}
            </el-menu-item>
          </template>
        </el-menu>
      </div>
