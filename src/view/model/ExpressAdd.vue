    ...
    <el-form :model="express" ref="expressForm" :rules="rule">
      ...
      <div style="line-height:25px;">
        <template v-for="c in cols">
          <button @click="selectCol(c.col)" style="margin-right:5px;" type="button" :key="c.col" :name="c.col"
                  class="btn btn-primary btn-xs">{{ c.col }}
          </button>
        </template>
      </div>
      ...
      this.$refs['expressForm'].validate(v => {
        if (v) {
          ...
          if (ts.isupdate) {
            ...
            var o = {
              id: cid.id, text: ts.express.kpiaggre + '(' + ts.express.kpiname + ')',
              li_attr: {
                tp: 'kpi', calc: true, drag: true, aggre: ts.express.kpiaggre, col: ts.express.expression,
                alias: ts.express.alias, dispName: ts.express.kpiname, tname: '', fmt: ts.express.kpifmt,
                unit: ts.express.kpiunit, kpinote: ts.express.kpinote, calcKpi: 1
              }, icon: 'fa fa-circle icon_kpi'
            }
